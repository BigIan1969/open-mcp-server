...
maybeTest("cdp: invalid session returns soft error", async () => {
  ...
  expect(res.error).toMatch(/Unknown sessionId/);
});

maybeTest("cdp: invalid method fails with valid session", async () => {
  ...
  // Now send invalid method
  await expect(
    server.rpc("cdp.send", {
      sessionId,
      method: "NonExistentDomain.method"
    })
  ).rejects.toThrow();
});
